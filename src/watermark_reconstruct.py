...
def estimate_normalized_alpha(J, W_m, num_images=31, threshold=170, invert=False, adaptive=False, adaptive_threshold=21, c2=10):
...
def solve_images(J, W_m, alpha, W_init, gamma=1, beta=1, lambda_w=0.005, lambda_i=1, lambda_a=0.01, iters=4):
    ...
    for _ in range(iters):
        ...
        for i in range(K):
            ...
            ax[0].imshow(PlotImage(J[i]))
            ax[1].imshow(PlotImage(Wk[i]))
            ax[2].imshow(PlotImage(Ik[i]))
            plt.draw()
            plt.pause(0.001)
            print(i)
